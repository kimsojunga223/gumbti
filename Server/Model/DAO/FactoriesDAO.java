		...
		Connection conn = null; 
	        ...
	        ResultSet rs = null;  
		        ...
		        try {
		        	...
		        	String SQL = "SELECT FAC_DISTRICT,"
		        			+ "RANK() OVER (ORDER BY CNT DESC) AS RANK "
		        			+ "FROM (SELECT FAC_DISTRICT, COUNT(*) CNT FROM FACTORY GROUP BY FAC_DISTRICT)";
	        ...
	        ResultSet rs = null;  
		        ...
		        try {
		        	...
		        	String SQL = "SELECT RANK "
		        			+ "FROM (SELECT FAC_DISTRICT, RANK() OVER (ORDER BY CNT DESC) AS RANK "
		        			+ "FROM (SELECT FAC_DISTRICT, COUNT(*) CNT FROM PARK  GROUP BY FAC_DISTRICT)) "
		        			+ "WHERE FAC_DISTRICT='"
		        			+ districtName
		        			+ "' ";

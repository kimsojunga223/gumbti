	  ...
	  public static ArrayList<String> getRankDistrictList() {
	        ...
	        ResultSet rs = null;  
		        ...
		        try {
		        	...
		        	String SQL = "SELECT PARK_DISTRICT,"
		        			+ "RANK() OVER (ORDER BY CNT DESC) AS RANK "
		        			+ "FROM (SELECT PARK_DISTRICT, COUNT(*) CNT FROM PARK GROUP BY PARK_DISTRICT)";
	        ...
	        ResultSet rs = null;  
		        ...
		        try {
		        	...
		        	String SQL = "SELECT RANK "
		        			+ "FROM (SELECT PARK_DISTRICT, RANK() OVER (ORDER BY CNT DESC) AS RANK "
		        			+ "FROM (SELECT PARK_DISTRICT, COUNT(*) CNT FROM PARK  GROUP BY PARK_DISTRICT)) "
		        			+ "WHERE PARK_DISTRICT='"
		        			+ districtName
		        			+ "' ";
